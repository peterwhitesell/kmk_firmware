...
class KBAKeyboard(KMKKeyboard):
  keymap = None
  def __init__(self, rows=0, columns=0, trackball=False, split=True, enable_rgb=True) -> None:
    super().__init__()
...
# ]]
  ...
  def assign_pins(self):
    ...
    self.row_pins = tuple((
      board.SCL, board.A3, board.A2, board.A1, board.A0,
    )[0:self.rows])
...
if __name__ == '__main__':
  ...
  if side == SplitSide.RIGHT:
    print('⌨️~ RIGHT')
    KBAKeyboard(5, 2, split=False, enable_rgb=True, trackball=False).go()
  else:
    print('⌨️~ LEFT')
    KBAKeyboard(3, 8).go()
  print('exit')